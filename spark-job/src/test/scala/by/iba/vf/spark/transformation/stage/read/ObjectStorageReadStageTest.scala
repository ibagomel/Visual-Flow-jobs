...
class ObjectStorageReadStageTest extends AnyFunSpec with PrivateMethodTester with MockitoSugar {
  it("COSReadStage") {
    ...
    val stage = new COSReadStage("id1", ObjectStorageReadCOSStageBuilder, cosConfig, optionsMap, "cos_storage")
  ...
  it("S3ReadStage") {
    ...
    val spark = mock[SparkSession]
    val optionsMap = Map("a" -> "b", "b" -> "c")
    val stage = new COSReadStage("id1", ObjectStorageReadS3StageBuilder, s3Config, optionsMap, "cos_storage")
