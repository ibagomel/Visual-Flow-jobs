...
object ObjectStorageReadCOSStageBuilder extends ReadStageBuilder {
  ...
  override protected def convert(config: Node): Stage =
    new COSReadStage(config.id, this, new COSConfig(config), getOptions(config.value), COSConfig.cosStorage)
}
...
object ObjectStorageReadS3StageBuilder extends ReadStageBuilder {
  ...
  override protected def convert(config: Node): Stage =
    new COSReadStage(config.id, this, new S3Config(config), getOptions(config.value), S3Config.cosStorage)
}
